...
def launch_setup(context, *args, **kwargs):
    ...
    rsp_node = Node(
        ...
        remappings=[
            ('robot_description', camera_name_val+'_description'),
            ('/tf', tf_topic),
    ...
    if(container_name_val == ''):
        ...
        if distro == 'foxy':
            ...
            arguments_val=[
                '--use_multi_threaded_executor', '--ros-args', '--log-level', 'info',
                '-r', ['/tf:=', tf_topic],
                '-r', ['/tf_static:=', tf_static_topic]
            ]
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                'tf_topic',
                default_value='/tf',
                description='TF topic name. Remap from `/tf` to a custom topic if needed.'),
            DeclareLaunchArgument(
                'tf_static_topic',
                default_value='/tf_static',
                description='Static TF topic name. Remap from `/tf_static` to a custom topic if needed.'),
            DeclareLaunchArgument(
                'xacro_path',
